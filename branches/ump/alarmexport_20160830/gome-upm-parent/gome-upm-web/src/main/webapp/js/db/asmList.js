...
$(function(){
	...
	$(".search_btn").bind("click", function(){
		...
		var diskGroup = $.trim($("#diskGroup").val());
		if(diskGroup != "" && diskGroup != null){
			//如果包含%、_，则将其替换为 \%、\_  (%、_在sql中属于特殊字符)
			if(diskGroup.indexOf("%") != -1){
				diskGroup = diskGroup.replace(/%/g, "\\%");
			}else if(diskGroup.indexOf("_") != -1){
				diskGroup = diskGroup.replace(/_/g, "\\_");
			}
			content.diskGroup=diskGroup;
		}
		content.search=true;
