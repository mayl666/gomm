...
/**
 * 服务器监控分析定时任务类-----zabbix生产服务器
 */
public class CpuAndMemoryProductTimer {
	private static final Logger logger = LoggerFactory.getLogger(CpuAndMemoryProductTimer.class);
	...
	public void work() {
		try {
			...
			logger.info(df.format(date)+"CpuAndMemoryProductTimer--cpu--执行开始啦！！！！！！"+date+"--------"+((df1.parse(df.format(date)).getTime()/1000)-3600));
			//查询当前时间点CPU的高于80%
			serverItemDetail.setKey_("cpu_use_all");
			//查询当前时间点Memory的值高于90%
			String analyzeMemoryValue = AppConfigUtil.getStringValue("server.analyze.memory");
			List<ServerItemDetail> cpuList  = serverAnalysisService.queryProCpuValueList(serverItemDetail);
			if(cpuList!=null && cpuList.size()>0){
				for (int i = 0; i < cpuList.size(); i++) {
					...
					List<ServerItemDetail> memryListCom  = serverAnalysisService.queryProCpuValueList(serverItemDetail);
					if(memryListCom!=null && memryListCom.size()>0){
						...
						List<ServerItemDetail> memryList  = serverAnalysisService.queryProCpuValueList(serverItemDetail);
						if(memryList!=null && memryList.size()>0){
							memryList.get(0).setvType("3");
							ServerItemDetail memory = serverAnalysisService.queryItems(memryList.get(0));
							if(memory==null){
								serverAnalysisService.addItem(memryList.get(0));
							}
							serverAnalysisService.addItemValuePro(memryList.get(0));
						}
					}
					//新增cpu的监控值
					cpuList.get(i).setvType("3");
					ServerItemDetail cpu = serverAnalysisService.queryItems(cpuList.get(i));
					if(cpu==null){
						serverAnalysisService.addItem(cpuList.get(i));
					}
					serverAnalysisService.addItemValuePro(cpuList.get(i));
					//查询服务器基本信息
					HostsInfo hostsInfo = serverAnalysisService.queryHostPro(cpuList.get(i).getHostid());
			...
			System.out.println("CpuAndMemoryProductTimer--cpu--执行完毕啦！！！！！！"+date);
			logger.info("CpuAndMemoryProductTimer--内存--执行开始啦！！！！！！"+date+"--------"+new Date(((df1.parse(df.format(date)).getTime()/1000)-3600)*1000l));
			...
			List<ServerItemDetail> memryList  = serverAnalysisService.queryProCpuValueList(itemDetail);
			if(memryList!=null && memryList.size()>0){
				for (int i = 0; i < memryList.size(); i++) {
					...
					List<ServerItemDetail> cpuListnewCom  = serverAnalysisService.queryProCpuValueList(itemDetail);
					if(cpuListnewCom!=null && cpuListnewCom.size()>0){
						logger.info("cpu使用率高于80%和当前时间点Memory的值高于90%--不进行cpu数据保存！！");
					}else{
						//新增cpu的监控值
						itemDetail.setValue(0.0000);
						List<ServerItemDetail> cpuListnew  = serverAnalysisService.queryProCpuValueList(itemDetail);
						if(cpuListnew!=null && cpuListnew.size()>0){
							cpuListnew.get(0).setvType("3");
							ServerItemDetail cpu = serverAnalysisService.queryItems(cpuListnew.get(0));
							if(cpu==null){
								serverAnalysisService.addItem(cpuListnew.get(0));
							}
							serverAnalysisService.addItemValuePro(cpuListnew.get(0));
						}
					}
					//新增Memory的监控值
					memryList.get(i).setvType("3");
					ServerItemDetail memory = serverAnalysisService.queryItems(memryList.get(i));
					if(memory==null){
						serverAnalysisService.addItem(memryList.get(i));
					}
					serverAnalysisService.addItemValuePro(memryList.get(i));
					
					//查询服务器基本信息
					HostsInfo hostsInfo = serverAnalysisService.queryHostPro(memryList.get(i).getHostid());
					hostsInfo.setvType("3");
					HostsInfo hostsInfoOld = serverAnalysisService.queryHostsInfo(hostsInfo);
					if(hostsInfoOld!=null){
						serverAnalysisService.updateHostsInfo(hostsInfo);
			...
			System.out.println("CpuAndMemoryProductTimer--内存--执行完毕啦！！！！！！"+date);
		} catch (Exception e) {
			e.printStackTrace();
		}
		 ...
		 * 定时更新服务器的基本属性信息
		 */
		List<HostsInfo> hostsList = serverAnalysisService.queryHostsInfoList("3");
