$(function(){
	networkMonitorPortError.service.init();
});


var networkMonitorPortError = {
		service : {
			init : function(){
				networkMonitorPortError.service.clickDeviceDetail();
				networkMonitorPortError.service.groupSelectChange();
				networkMonitorPortError.service.search();
				networkMonitorPortError.controller.search(null,null,null,null,1);
				networkMonitorPortError.service.bindPageClick();
			},
			...
			groupSelectChange : function(){
				$("#groupSelect").change(function(){
					...
					if(groupId != ""){
						networkMonitorPortError.controller.getDevices(groupId);
					}else{
						$("#deviceSelect").val("全部");
			...
			bindPageClick : function(){
			    $(".list_table").on("click",".pageNumber", function(){
					...
					networkMonitorPortError.controller.search(sdate,edate,groupId,deviceId,pageNo);
			    });
			},
			search : function(){
				$("#portSearchBtn").click(function(){
					...
					networkMonitorPortError.controller.search(sdate,edate,groupId,deviceId,1);
				});
			}
		...
		controller : {
			...
			search : function(sdate,edate,groupId,deviceId,pageNo){
				var pageSize = 10;
				$.ajax({
					url:contextPath+'/prtg/port/error/table',
					type:'POST',
					dataType : 'html',	
