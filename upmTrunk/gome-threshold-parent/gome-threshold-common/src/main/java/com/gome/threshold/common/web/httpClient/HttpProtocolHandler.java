package com.gome.threshold.common.web.httpClient;
...
import com.gome.threshold.common.MshopRpcRuntimeException;
import com.gome.threshold.constants.Constants;
...
public class HttpProtocolHandler {
	...
	private static String DEFAULT_CHARSET = Constants.UTF8;
	...
	public HttpResponse execute(HttpRequest request, String strParaFileName, String strFilePath)
		...
		if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
			...
			logger.info("http {} request:{}", HttpRequest.METHOD_GET, method.getURI().toString());
		} else if (strParaFileName.equals("") && strFilePath.equals("")) {
			// post模式且不带上传文件
			method = new PostMethod(request.getUrl());
			((PostMethod) method).getParams().setParameter(HttpMethodParams.RETRY_HANDLER,
					new DefaultHttpMethodRetryHandler());
			((PostMethod) method).addParameters(request.getParameters());
			method.addRequestHeader("Content-Type",
					"application/x-www-form-urlencoded; text/html; charset=" + charset);
			logger.info("http {} request:{}", HttpRequest.METHOD_POST, method.getURI().toString());
		} else {
			// post模式且带上传文件
			method = new PostMethod(request.getUrl());
		...
		if(StringUtils.isNotEmpty(request.getAccept())){
			method.addRequestHeader("Accept", request.getAccept());
		}
		...
		try {
			...
			logger.info("http response:{}", response.getStringResult());
		} catch (UnknownHostException ex) {
			logger.error("http error:{} , {}", ex.getMessage(), ex.getCause());
			throw new MshopRpcRuntimeException("远程调用异常", ex.getCause());
