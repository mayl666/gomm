...
$(function(){
    			...
    			if(data.code==1){
    				...
    				for(var i=1; i<=picNum; i++){
    					if(disArray.indexOf(i) < 0){
    						...
    						html = html + "<option value=" + i + " disabled='disabled'>" + i + " 【已占】</option>";
    					}
    				}
    				$("#sort").html(html);
    			}else{
	...
	$("#save_btn").click(function(){
    	...
    	$.ajax({
    		...
    		success:function(data){
    			if(data.code==7){
    				pop_up("该频道下的广告位已满，无法添加",false);
    			} else if(data.code==10){
    				pop_up("该频道下的相应位置已存在广告位，无法添加",false);
    			} else if(data.code==1){
    				pop_up("添加成功",true);
	...
	function checkLinks(){
    	...
    	//var urlReg = /^(https|http):\/\/([A-z0-9]+[_\-]?[A-z0-9]+\.)*[A-z0-9]+\-?[A-z0-9]+\.[A-z]{2,}(\/.*)*\/?$/;
